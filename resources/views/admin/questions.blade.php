...
@section('page-title', 'Questions &mdash; {{ $examination->title ?? '' }}')
...
<div class="card" data-aos="fade-up">
    ...
    <div class="overflow-x-auto" x-data="tableData({{ json_encode($questions->items()) }}, [
        {label: 'ID', field: 'id'},
        {label: 'Question', field: 'question'},
        {label: 'Type', field: 'type'},
        {label: 'Answer', field: 'answer'},
        {label: 'A', field: 'choice_1'},
        {label: 'B', field: 'choice_2'},
        {label: 'C', field: 'choice_3'},
        {label: 'D', field: 'choice_4'},
            ...
            <tbody>
                ...
                <template x-for="(row, idx) in paged" :key="idx">
                    <tr class="border-t border-white/5 hover:bg-white/5">
                        <template x-for="col in columns" :key="col.field">
                            <td class="py-3 px-3">
                                <span x-show="!col.badge" x-text="row[col.field] || '—'" :class="col.field === 'question' ? 'max-w-xs truncate block' : 'text-gray-400'"></span>
                                <span x-show="col.badge" x-text="badgeLabel(row[col.field])"
                                      :class="badgeClass(row[col.field])"></span>
                            </td>
                        </template>
                        <td class="py-3 px-3">
                            <div class="flex gap-2">
                                <form method="POST" action="{{ url('/admin/examinations/manage/approve') }}" class="inline">
                                    @csrf
                                    <input type="hidden" name="id" :value="row.id">
                                    <button type="submit" class="text-xs px-2 py-1 rounded" :class="badgeClass('1')">Approve</button>
                                </form>
                                <form method="POST" action="{{ url('/admin/examinations/manage/reject') }}" class="inline">
                                    @csrf
                                    <input type="hidden" name="id" :value="row.id">
